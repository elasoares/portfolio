...
export function FormularioPerfil() {
  ...
  const schemaValidationForm = yup.object({
    ...
    date: yup.string()
      ...
      .max(50, "O campo data deve conter no máximo cinquenta caracteres."),
    hour: yup.string()
  ...
  const postar = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage
        .from("imagens")
        .upload(fileName, selecionarArquivo);
      ...
      const { error: insertError } = await supabase
        .from("posts")
        .insert([{
          about, title, image_url: urlData.publicUrl,
          tecnologia: saveTech, functionality,
          tipo: select, link, institute, date, hour
        }]);
  ...
  const handleSelectChange = (event) => {
    setCertificado(event.target.value === "certificado");
  };
  ...
  return (
    <Formik
      ...
      initialValues={{
        title: "", select: "", about: "", functionality: "",
        tecnologia: "", link: "", date: "", hour: "", institute: "",
      }}
      onSubmit={postar}>

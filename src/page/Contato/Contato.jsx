...
const signUpSchema = yup.object({
  ...
  subject: yup.string()
    ...
    .max(200, "O campo assunto precisa no máximo duzentos caracteres.")
...
export function Contato() {
  ...
  const enviarMensagem = async (values, { setSubmitting, resetForm }) => {
    ...
    <div className={styles.containerContato}>
      <div className={styles.formContainer}>
        <Formik
          ...
          initialValues={{ name: "", email: "", subject: "", message: "", acceptTerms: false }}
          ...
          {({ handleBlur, handleChange, handleSubmit, errors, values, touched }) => (
            <Card>
              <form onSubmit={handleSubmit} noValidate className={`${styles.formulario}`}>
                ...
                <p className={styles.errorFormk}>{touched.name && errors.name}</p>

                <fieldset>
                  <TextField name="email" type="email" placeholder="Digite seu e-mail"
                    onChange={handleChange} onBlur={handleBlur} value={values.email} />
                </fieldset>
                <p className={styles.errorFormk}>{touched.email && errors.email}</p>

                <fieldset>
                  <TextField name="subject" type="text" placeholder="Digite o assunto"
                    onChange={handleChange} onBlur={handleBlur} value={values.subject} />
                </fieldset>
                <p className={styles.errorFormk}>{touched.subject && errors.subject}</p>

                <fieldset>
                  <textarea name="message" id="message" rows="3" cols="27"
                    placeholder="Escreva sua mensagem aqui"
                    onChange={handleChange} onBlur={handleBlur} value={values.message}
                    className={styles.textAreaMessagem}></textarea>
                </fieldset>
                <p className={styles.errorFormk}>{touched.message && errors.message}</p>

                <fieldset className={styles["termo-container"]}>
                  <input type="checkbox" name="acceptTerms" checked={values.acceptTerms}
                    onChange={handleChange} className={styles["input-termo"]} />
                ...
                <p className={styles.errorFormk}>{touched.acceptTerms && errors.acceptTerms}</p>

                <Button className={styles.botao} type="submit">
                  Enviar mensagem <GrSend />
                </Button>
              </form>
            </Card>
          )}
